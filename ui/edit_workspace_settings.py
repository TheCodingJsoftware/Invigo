...
from PyQt6.QtWidgets import QAbstractItemView, QCheckBox, QComboBox, QDialog, QGroupBox, QHBoxLayout, QInputDialog, QLabel, QLineEdit, QListWidget, QMenu, QMessageBox, QPlainTextEdit, QPushButton, QTableWidget, QTableWidgetItem, QTabWidget, QTextEdit, QWidget
...
class TagWidget(QWidget):
    ...
    def __init__(self, tag: Tag, remaining_tags: list[Tag], workspace_settings: WorkspaceSettings, parent: "FlowTagWidget"):
...
class FlowTagWidget(QWidget):
    def __init__(self, flow_tag: FlowTag, workspace_settings: WorkspaceSettings, parent: QWidget):
        super(FlowTagWidget, self).__init__(parent)
        self.parent = parent
...
class FlowTagsTableWidget(QTableWidget):
    def __init__(self, flow_tag_group: FlowTags, workspace_settings: WorkspaceSettings, parent: QWidget):
        super(FlowTagsTableWidget, self).__init__(parent)
...
class EditWorkspaceSettings(QDialog):
    ...
    def delete_tag(self):
        tag_name, ok = QInputDialog.getItem(self, "Delete tag", "Select a tag to delete", self.workspace_settings.get_all_tags(), self.workspace_settings.get_all_tags().index(self.get_selected_tag().name), False)
        if tag_name and ok:
    ...
    def rename_tag(self):
        ...
        new_name, ok = QInputDialog.getText(self, "Rename tag", "Enter a new name for the selected tag:", text=current_tag.name)
        if new_name and ok:
    ...
    def load_status_table(self):
        if selected_tag := self.get_selected_tag():
            ...
            for row, status in enumerate(selected_tag.statuses):
                ...
                checkbox_moves_tag_forward.stateChanged.connect(partial(self.checkbox_move_tag_forward_changed, checkbox_moves_tag_forward))
