...
class EditLaserCutPart(QDialog):
    def __init__(self, laser_cut_part: LaserCutPart, parent=None):
        super(EditLaserCutPart, self).__init__(parent)
...
class LaserCutPartsTabWidget(CustomTabWidget):
    def __init__(self, parent: QWidget) -> None:
        super(LaserCutPartsTabWidget, self).__init__(parent)
...
class PaintWidget(QWidget):
    def __init__(self, laser_cut_part: LaserCutPart, paint_settings_widget: PaintSettingsWidget, parent: LaserCutPartsTableWidget) -> None:
...
class LaserCutTab(QWidget):
    def __init__(self, parent) -> None:
        ...
        self.table_laser_cut_parts_widgets: dict[LaserCutPart, dict[str, QTableWidgetItem]] = {}
    ...
    def edit_category(self):
        if self.category.name in ["Recut", "Custom"]:
            return
        edit_dialog = EditCategoryDialog(f"Edit {self.category.name}", f"Delete, duplicate, or rename: {self.category.name}.", self.category.name, self.category, self.laser_cut_inventory, self)
        if edit_dialog.exec():
            ...
            if action == "DUPLICATE":
                ...
                if new_name == self.category.name:
                    new_name += " - Copy"
                new_category = self.laser_cut_inventory.duplicate_category(self.category, new_name)
                self.laser_cut_inventory.add_category(new_category)
                table = LaserCutPartsTableWidget(self.tab_widget)
                self.category_tables.update({new_category: table})
    ...
    def load_categories(self):
        ...
        all_categories = [category.name for category in self.laser_cut_inventory.get_categories()]
    ...
    def load_table(self):
        ...
        row_index = 0
        grouped_laser_cut_parts = self.laser_cut_inventory.get_group_categories(self.laser_cut_inventory.get_laser_cut_parts_by_category(self.category))
        for group, laser_cut_parts in grouped_laser_cut_parts.items():
            ...
            for laser_cut_part in laser_cut_parts:
                ...
                current_table.setItem(row_index, current_table.shelf_number_column, table_item_shelf_number)
        ...
        if laser_cut_parts := self.get_selected_laser_cut_parts():
            ...
            if set_custom_limit_dialog.exec():
                for laser_cut_part in laser_cut_parts:
                    ...
                    if laser_cut_part.quantity <= laser_cut_part.red_quantity_limit:
                        self.set_table_row_color(current_table, self.table_laser_cut_parts_widgets[laser_cut_part]["row"], "#3F1E25")
                    elif laser_cut_part.quantity <= laser_cut_part.yellow_quantity_limit:
                        self.set_table_row_color(current_table, self.table_laser_cut_parts_widgets[laser_cut_part]["row"], "#413C28")
                    else:
                        self.set_table_row_color(current_table, self.table_laser_cut_parts_widgets[laser_cut_part]["row"], "#2c2c2c")
                self.laser_cut_inventory.save()
                self.sync_changes()
