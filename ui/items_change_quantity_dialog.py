...
class ItemsChangeQuantityDialog(QDialog):
    def __init__(
        ...
        super(ItemsChangeQuantityDialog, self).__init__(parent)
        uic.loadUi("ui/items_change_quantity_dialog.ui", self)
        ...
        if self.items:
            self.pushButton_category.clicked.connect(lambda: (self.pushButton_item.setChecked(False), self.quantity_changed()))
            self.pushButton_item.clicked.connect(lambda: (self.pushButton_category.setChecked(False), self.quantity_changed()))
        else:
    ...
    def quantity_changed(self):
        settings_file.set_value("change_quantities_by", "Item" if self.pushButton_item.isChecked() else "Category")
