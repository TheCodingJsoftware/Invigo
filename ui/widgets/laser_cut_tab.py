...
class EditLaserCutPart(QDialog):
    def __init__(self, laser_cut_part: LaserCutPart, parent=None):
        ...
        layout.addLayout(grid_layout)  # Add the grid layout to the scrollable widget's layout
...
class LaserCutTab(QWidget, Ui_Form):
    def __init__(self, parent):
        ...
        self.parent: MainWindow = parent
        self.laser_cut_parts_inventory: LaserCutInventory = self.parent.laser_cut_parts_inventory
    ...
    def load_ui(self):
        ...
        autofill_search_options = natsorted(self.laser_cut_parts_inventory.get_all_part_names())
    ...
    def remove_category(self):
        category_to_remove, ok = QInputDialog.getItem(
            ...
            [category.name for category in self.laser_cut_parts_inventory.get_categories()],
        ...
        if category_to_remove and ok:
            category = self.laser_cut_parts_inventory.delete_category(category_to_remove)
        ...
        if edit_dialog.exec():
            ...
            if action == "DUPLICATE":
                ...
                if new_name == self.category.name:
                    new_name += " - Copy"
                new_category = self.laser_cut_parts_inventory.duplicate_category(self.category, new_name)
                self.laser_cut_parts_inventory.add_category(new_category)
                table = LaserCutPartsTableWidget(self.tab_widget)
                self.category_tables.update({new_category: table})
                ...
                self.laser_cut_parts_inventory.rename_category(self.category, input_text)
    ...
    def load_categories(self):
        ...
        all_categories = [category.name for category in self.laser_cut_parts_inventory.get_categories()]
        try:
            tab_order: list[str] = self.settings_file.get_value("category_tabs_order")["Laser Cut Inventory"]
        except KeyError:
            tab_order = []
    ...
    def update_laser_cut_part(self, laser_cut_part_data: dict):
        if update_laser_cut_part := self.laser_cut_parts_inventory.update_laser_cut_part_data(laser_cut_part_data["id"], laser_cut_part_data):
            with contextlib.suppress(
                ...
                self.update_laser_cut_part_table(self.category_tables[self.category], update_laser_cut_part)
        else:  # Meaning the laser_cut_part just got added
            # I don't think this will ever run.
            laser_cut_part = LaserCutPart(laser_cut_part_data, self.laser_cut_parts_inventory)
    ...
    def load_table(self):
        self.category: Category = self.laser_cut_parts_inventory.get_category(self.tab_widget.tabText(self.tab_widget.currentIndex()))
        ...
        row_index = 0
        grouped_laser_cut_parts = self.laser_cut_parts_inventory.get_group_categories(self.laser_cut_parts_inventory.get_laser_cut_parts_by_category(self.category))
    ...
    def update_category_total_stock_costs(self):
        summary: dict[str, float] = {
            category.name: self.laser_cut_parts_inventory.get_category_parts_total_stock_cost(category) for category in self.laser_cut_parts_inventory.get_categories()
    ...
    def change_quantities(self, add_or_remove: Literal["ADD", "REMOVE"]):
        ...
        dialog = ItemsChangeQuantityDialog(self.category.name, add_or_remove, selected_laser_cut_parts, self)
        if dialog.exec():
    ...
    def ui_update_laser_cut_part(self, item_dialog: EditLaserCutPart, laser_cut_part: LaserCutPart):
    ...
    def get_selected_laser_cut_parts(self) -> list[LaserCutPart]:
        ...
        selected_laser_cut_parts.extend(laser_cut_part for laser_cut_part, table_items in self.table_laser_cut_parts_widgets.items() if table_items["row"] in selected_rows)
    ...
    def save_current_tab(self):
        if self.finished_loading:
            self.parent.laser_cut_tab_widget_last_selected_tab_index = self.tab_widget.currentIndex()

    def restore_last_selected_tab(self):
        if self.tab_widget.currentIndex() == self.parent.laser_cut_tab_widget_last_selected_tab_index:
            self.sort_laser_cut_parts()  # * This happens when the last selected tab is the first tab
        else:
            self.tab_widget.setCurrentIndex(self.parent.laser_cut_tab_widget_last_selected_tab_index)
