...
class WorkspaceTab(QWidget):
    def __init__(
        ...
        super().__init__(parent)
        uic.loadUi("ui/widgets/workspace_tab.ui", self)
        self.parent = parent
    ...
    def load_planning_assembly_items_table(
        ...
        def add_item(row_index: int, group: str):
            ...
            table.setItem(
                ...
                col_index,
                QTableWidgetItem(
                    f'{item_group.get_item(group).starting_date} to {item_group.get_item(group).ending_date}, ({QDate.fromString(item_group.get_item(group).starting_date, "yyyy-MM-dd").daysTo(QDate.fromString(item_group.get_item(group).ending_date, "yyyy-MM-dd"))} days)'
                ),
            )  # 0
            table.resizeColumnsToContents()
