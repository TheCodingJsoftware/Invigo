...
class PopoutWidget(QWidget):
    def __init__(self, layout_to_popout: QVBoxLayout, parent=None):
        ...
        self.original_layout_parent: "StructuralSteelSettingsTab" = self.original_layout.parentWidget()
...
class StructuralSteelSettingsTab(QWidget, Ui_Form):
    ...
    def load_tabs(self):
        ...
        self.pounds_per_cubic_foot_table.cellChanged.connect(self.pounds_per_cubic_foot_table_changed)
    ...
    def load_price_per_pound_table(self):
        ...
        for row, (material, price_per_pound) in enumerate(self.structural_steel_settings.material_price_per_pounds):
    ...
    def load_pounds_per_cubic_foot_table(self):
        ...
        for row, (material, pounds_per_cubic_foot) in enumerate(self.structural_steel_settings.material_densities):
            self.pounds_per_cubic_foot_table.insertRow(row)
            table_item_material = QTableWidgetItem(material.name)
            table_item_material.setFont(self.tables_font)
            self.pounds_per_cubic_foot_table.setItem(row, 0, table_item_material)

            table_item_pounds_per_cubic_foot = QTableWidgetItem(f"{pounds_per_cubic_foot.density:,.3f}")
            table_item_pounds_per_cubic_foot.setFont(self.tables_font)
            self.pounds_per_cubic_foot_table.setItem(row, 1, table_item_pounds_per_cubic_foot)
            ...
            table_item_latest_change.setFont(self.tables_font)
            self.pounds_per_cubic_foot_table.setItem(row, 2, table_item_latest_change)
    ...
    def pounds_per_cubic_foot_table_changed(self):
        for (
            ...
            table_item_pound,
        ) in self.pounds_per_cubic_foot_table_items.items():
            new_value = float(table_item_pound.text().strip().replace(",", ""))
        ...
        if self.tabWidget.tabText(self.tabWidget.currentIndex()) == "Price Per Pound":
            for row in range(self.price_per_pound_table.rowCount()):
                if self.price_per_pound_table.item(row, 0).text() == self.materials_list.selectedItems()[0].text():
                    ...
                    return
        elif self.tabWidget.tabText(self.tabWidget.currentIndex()) == "Pounds Per Cubic Foot":
            for row in range(self.pounds_per_cubic_foot_table.rowCount()):
                if self.pounds_per_cubic_foot_table.item(row, 0).text() == self.materials_list.selectedItems()[0].text():
    ...
    def remove_material(self):
        try:
            selected_row = self.structural_steel_settings.get_materials().index(self.materials_list.selectedItems()[0].text())
        except IndexError:
            selected_row = 0
    ...
    def rename_material(self):
        selected_material = self.structural_steel_settings.materials.get(self.materials_list.selectedItems()[0].text())
