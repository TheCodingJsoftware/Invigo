...
class SheetEditor(QMainWindow):
    ...
    def removeMaterial(self):
        ...
        if ok and material:
            if foundItems := self.materialsList.findItems(
                material, Qt.MatchFlag.MatchExactly
            ):
                item = foundItems[0]
    ...
    def removeThickness(self):
        thicknesses = [self.thicknessesList.item(i).text() for i in range(self.thicknessesList.count())]
        thickness, ok = QInputDialog.getItem(self, "Remove Thickness", "Select a thickness to remove:", thicknesses, 0, False)
        if ok and thickness:
            if foundItems := self.thicknessesList.findItems(
                thickness, Qt.MatchFlag.MatchExactly
            ):
                item = foundItems[0]
