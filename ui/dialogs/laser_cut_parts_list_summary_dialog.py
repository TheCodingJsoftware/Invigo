...
class LaserCutPartsListSummaryDialog(QDialog, Ui_Form):
    def __init__(self, assemblies: list[Assembly], parent):
        ...
        self.laser_cut_part_table_items: dict[WorkspaceLaserCutPartGroup, dict[str, QTableWidgetItem | QComboBox | QWidget | int | FlowtagDataButton]] = {}
        ...
        for laser_cut_part, part_data in all_assembly_laser_cut_parts.items():
            for workspace_laser_cut_part_group in grouped_laser_cut_parts:
                if workspace_laser_cut_part_group.base_part.name == laser_cut_part.name:
                    for _ in range(part_data["assembly_quantity"]):
    ...
    def add_workspace_laser_cut_part_to_table(self, workspace_laser_cut_part_group: WorkspaceLaserCutPartGroup):
        ...
        try:
            ...
            pixmap = image.scaled(new_width, self.laser_cut_parts_table.row_height, Qt.AspectRatioMode.KeepAspectRatio)
