...
class AddComponentDialog(QDialog):
    def __init__(self, parent) -> None:
        ...
        self.comboBox_part_number.addItems(natsorted(self.components_inventory.get_all_part_numbers()))
        self.comboBox_part_number.setCurrentText("")
        self.comboBox_part_number.lineEdit().textChanged.connect(self.part_number_changed)
        ...
        self.listWidget_components.addItems(natsorted(self.components_inventory.get_all_part_names() + self.components_inventory.get_all_part_numbers()))
    ...
    def selection_changed(self):
        for component in self.components_inventory.components:
            if component.part_name == self.listWidget_components.currentItem().text() or component.part_number == self.listWidget_components.currentItem().text():
    ...
    def part_number_changed(self) -> None:
        for component in self.components_inventory.components:
            if component.part_number == self.comboBox_part_number.currentText():
                self.listWidget_components.blockSignals(True)
                self.listWidget_components.setCurrentRow(self.get_component_row(component.part_number))
                self.listWidget_components.blockSignals(False)
                self.comboBox_component_name.lineEdit().blockSignals(True)
                self.comboBox_component_name.setCurrentText(component.part_name)
                self.comboBox_component_name.lineEdit().blockSignals(False)
