    ...
    def quantity_changed(self):
        ...
        if self.pushButton_category.isChecked():
            ...
            for i, item in enumerate(self.items):
                if self.add_or_remove == "ADD":
                    items_string += f"  {i + 1}. {item.part_name if isinstance(item, Component) else item.name}\n\twill go from {item.inventory_data.quantity if isinstance(item, LaserCutPart) else item.quantity} to {item.inventory_data.quantity + self.doubleSpinBox_quantity.value() if isinstance(item, LaserCutPart) else item.quantity + self.doubleSpinBox_quantity.value()}\n"
