...
sheets_inventory = JsonFile(file_name=f"data/{inventory_file_name} - Price of Steel")
laser_cut_inventory = JsonFile(file_name=f"data/{inventory_file_name} - Parts in Inventory")
...
def add_parts(batch_data: dict, parts_to_add: list[str]):
    ...
    for category in list(laser_cut_inventory.get_data().keys()):
        if category == "Recut":
            continue
...
def add_part_to_inventory(category, part_to_add, batch_data) -> None:
    laser_cut_inventory.add_item_in_object(category, part_to_add)
    ...
    laser_cut_inventory.change_object_item(category, part_to_add, batch_data[part_to_add])
...
def update_quantity(batch_data: dict, part_name_to_update: str, quantity: int) -> None:
    for category in list(laser_cut_inventory.get_data().keys()):
        ...
        if part_exists(category=category, part_name_to_find=part_name_to_update):
            current_quantity: int = laser_cut_inventory.get_data()[category][part_name_to_update]["current_quantity"]
            batch_data[part_name_to_update]["current_quantity"] = current_quantity + quantity
            batch_data[part_name_to_update]["unit_quantity"] = laser_cut_inventory.get_data()[category][part_name_to_update]["unit_quantity"]
            ...
            laser_cut_inventory.change_object_item(category, part_name_to_update, batch_data[part_name_to_update])
...
def add_recut_parts(batch_data: dict, recut_parts: list[str]) -> None:
    for recut_part in recut_parts:
        ...
        if part_exists(category="Recut", part_name_to_find=recut_part):
            recut_count = laser_cut_inventory.get_data()["Recut"][recut_part]["recut_count"] + 1
            laser_cut_inventory.change_object_in_object_item(
                "Recut",
                name,
                "recut_count",
                recut_count,
            ...
            batch_data[recut_part]["recut_count"] = recut_count
        laser_cut_inventory.add_item_in_object("Recut", name)
        ...
        laser_cut_inventory.change_object_item("Recut", name, batch_data[recut_part])
...
def part_exists(category: str, part_name_to_find: str) -> bool:
    return part_name_to_find in list(laser_cut_inventory.get_data()[category].keys())
    ...
    for category in list(category_data.keys()):
        ...
        for sheet_name in list(category_data[category].keys()):
            if sheet_name_to_update == sheet_name:
                ...
                sheets_inventory.change_object_in_object_item(category, sheet_name, "current_quantity", old_quantity - sheet_count)
                ...
                if (old_quantity - sheet_count) <= red_quantity_limit:
                    ...
                    if category != "Cutoff" and not is_order_pending and not has_sent_warning:
                        ...
                        sheets_inventory.change_object_in_object_item(category, sheet_name_to_update, "has_sent_warning", True)

                if category == "Cutoff" and old_quantity - sheet_count == 0:
                    sheets_inventory.remove_object_item(category, sheet_name)
                    CustomPrint.print(
...
def get_cutoff_sheets() -> dict:
    sheets_inventory.load_data()
    return sheets_inventory.get_data()["Cutoff"]
...
def add_sheet(
    ...
    sheets_inventory.load_data()
    sheets_inventory.add_item_in_object(category_name, sheet_name)
    sheets_inventory.change_object_in_object_item(category_name, sheet_name, "current_quantity", sheet_count)
    sheets_inventory.change_object_in_object_item(category_name, sheet_name, "sheet_dimension", sheet_dim)
    sheets_inventory.change_object_in_object_item(category_name, sheet_name, "thickness", thickness)
    sheets_inventory.change_object_in_object_item(category_name, sheet_name, "material", material)
    sheets_inventory.change_object_in_object_item(category_name, sheet_name, "group", material)
    sheets_inventory.change_object_in_object_item(
        category_name,
        sheet_name,
        "latest_change_current_quantity",
...
def remove_cutoff_sheet(sheet_name: str, _connected_clients):
    ...
    if sheet_exists(sheet_name):
        sheets_inventory.remove_object_item("Cutoff", sheet_name)
    CustomPrint.print(
        f'INFO - Removed "{sheet_name}" from Cutoff',
        ...
        for _sheet_name in list(category_data[category].keys()):
            if sheet_name == _sheet_name:
                ...
                if red_quantity_limit >= new_quantity:
                    sheets_inventory.change_object_in_object_item(category, sheet_name, "has_sent_warning", False)
                sheets_inventory.change_object_in_object_item(
                    category,
                    sheet_name,
                    "current_quantity",
