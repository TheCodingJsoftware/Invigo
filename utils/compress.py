...
def compress_database(path_to_file: str, on_close: bool = False) -> None:
    ...
    if on_close:
        path_to_zip_file: str = f"backups/{settings_file.get_value('inventory_file_name')} - {datetime.now().strftime('%B %d %A %Y %I-%M-%S %p')} - (Auto Generated).zip"
    else:
        path_to_zip_file: str = f"backups/{settings_file.get_value('inventory_file_name')} - {datetime.now().strftime('%B %d %A %Y %I-%M-%S %p')}.zip"
