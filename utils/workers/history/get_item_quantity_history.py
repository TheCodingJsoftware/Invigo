...
class GetItemQuantityHistoryWorker(BaseWorker):
    def __init__(
        self,
        item_type: Literal["sheet", "component", "structural_steel_item"],
        item_id: int,
    ):
        super().__init__(name="GetItemQuantityHistoryWorker")
        self.url = f"{self.DOMAIN}/get_quantity_history/{item_type}/{item_id}"
