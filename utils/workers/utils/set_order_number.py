...
class SetOrderNumberWorker(BaseWorker):
    def __init__(self, order_number: float):
        super().__init__(name="SetOrderNumberWorker")
        self.url = f"{self.DOMAIN}/set_order_number/{int(order_number)}"

    def do_work(self):
        self.logger.info(f"Requesting order number from {self.url}")
        try:
            with requests.Session() as session:
                response = session.post(self.url, headers=self.headers, timeout=10)
