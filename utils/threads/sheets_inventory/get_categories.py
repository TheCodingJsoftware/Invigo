...
class GetSheetCategoriesSignals(QObject):
    signal = pyqtSignal(object, int)  # (data or error, status_code)


class GetSheetCategoriesWorker(QRunnable):
    def __init__(self):
        super().__init__()
        self.signals = GetSheetCategoriesSignals()
        self.SERVER_IP = get_server_ip_address()
        self.SERVER_PORT = get_server_port()
        self.url = f"http://{self.SERVER_IP}:{self.SERVER_PORT}/sheets_inventory/get_categories"
        ...
        try:
            with requests.Session() as session:
                ...
                if isinstance(categories, list):
                    self.signals.signal.emit(categories, 200)
                else:
                    self.signals.signal.emit({"error": "Invalid format received"}, 500)
