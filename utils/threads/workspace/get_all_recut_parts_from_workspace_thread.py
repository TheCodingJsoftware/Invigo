...
class GetRecutPartsFromWorkspace(QThread):
    ...
    def __init__(self, job_id: int = None):
        ...
        if job_id is not None:
            self.url = f"http://{self.SERVER_IP}:{self.SERVER_PORT}/workspace/get_recut_parts_from_job/{job_id}"
        else:
            self.url = f"http://{self.SERVER_IP}:{self.SERVER_PORT}/workspace/get_all_recut_parts"
    ...
    def run(self):
        try:
            with requests.Session() as session:
                ...
                if isinstance(job_data, (list, dict)):
                    self.signal.emit(
                        job_data, 200
