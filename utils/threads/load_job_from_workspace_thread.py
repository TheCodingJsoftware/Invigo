...
class LoadJobFromWorkspaceThread(QThread):
    ...
    def __init__(self, job: Job, item: QTreeWidgetItem, job_id: int, laser_cut_inventory: LaserCutInventory, components_inventory: ComponentsInventory):
    ...
    def process_children(self, job: Job, parent: Assembly, children: list[dict[str, dict | list | str]]):
        ...
        for child in children:
            if child["type"] == "assembly":
                ...
                self.process_children(job, assembly, child.get("children", []))  # Recursive for sub-assemblies
            ...
            with requests.Session() as session:
                ...
                for child in job_data.get("children", []):
                    if child["type"] == "assembly":
                        ...
                        self.process_children(self.job, assembly, child.get("children", []))

                if isinstance(job_data, dict):
                    self.signal.emit(self.job, self.item, job_data, 200)  # Emit the job data with HTTP 200 status
                else:
                    self.signal.emit(self.job, self.item, {"error": "Invalid data format received"}, 500)

        except requests.exceptions.Timeout:
            self.signal.emit(self.job, self.item, {"error": "Request timed out"}, 408)
        except requests.exceptions.ConnectionError:
            self.signal.emit(self.job, self.item, {"error": "Could not connect to the server"}, 503)
        except requests.exceptions.HTTPError as e:
            self.signal.emit(self.job, self.item, {"error": f"HTTP Error: {str(e)}"}, response.status_code)
        except requests.exceptions.RequestException as e:
            self.signal.emit(self.job, self.item, {"error": f"Request failed: {str(e)}"}, 500)
        finally:
            self.finished.emit()
