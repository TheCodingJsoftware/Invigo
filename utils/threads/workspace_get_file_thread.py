...
class WorkspaceDownloadFile(QThread):
    ...
    def __init__(self, files_to_download: list[str], open_when_done: bool, download_directory: str = None):
        ...
        if not self.download_directory:
            self.download_directory = os.path.join(os.getcwd(), "data", "workspace")
    ...
    def run(self):
        for file_to_download in self.files_to_download:
            try:
                ...
                Path(f"{self.download_directory}/{file_ext}").mkdir(parents=True, exist_ok=True)
                if response.status_code == 200:
                    with open(f"{self.download_directory}/{file_ext}/{file_name}", "wb") as file:
