...
class AddJobToWorkspaceThread(QThread):
    ...
    def __init__(self, job: Job):
        ...
        self.job: Job = job
        self.url = (
            f"http://{self.SERVER_IP}:{self.SERVER_PORT}/workspace_add_job"
        )
    ...
    def run(self):
        ...
        try:
            ...
            response = requests.post(self.url, data=job_data, headers=headers, timeout=10)
            ...
            self.signal.emit(f"HTTP error: {http_err}", response.status_code if 'response' in locals() else 500)

        except requests.RequestException as req_err:
            self.signal.emit(f"Request error: {req_err}", 500)

        except msgspec.DecodeError:
            self.signal.emit("JSON parse error", response.status_code if 'response' in locals() else 500)
