...
class NestsTable:
    def __init__(self, job: Job) -> None:
        self.headers = ["Nest Name", "Thickness", "Material", "Qty", "Sheet Cut Time", "Nest Cut Time"]
        self.job = job
...
class LaserCutPartsTable:
    def __init__(self, laser_cut_parts: list[LaserCutPart]) -> None:
        self.headers = [
            "Part",
            "Material",
            "Thickness",
    ...
    def generate(self) -> str:
        ...
        for laser_cut_part in self.laser_cut_parts:
            ...
            <td class="small-text" data-column="3" data-name="qty">{laser_cut_part.quantity}</td>
            <td class="small-text min" data-column="4" data-name="process">{laser_cut_part.flow_tag.get_name()}</td>
        ...
        html += f"""<tfoot>
            <tr>
                ...
                <th class="small-text" data-column="4" data-name="process"></th>
                <th class="small-text" data-column="5" data-name="shelf-#"></th>
...
class ComponentsTable:
    def __init__(self, components: list[Component]) -> None:
        ...
        self.server_directory = f"http://{get_server_ip_address()}"
        self.headers = ["Part", "Part #", "Shelf #", "Qty", "Unit Price", "Price"]
    ...
    def generate(self):
        ...
        for component in self.components:
            ...
            <td class="small-text" data-column="3" data-name="qty">{component.quantity}</td>
            <td class="small-text" data-column="4" data-name="unit-price">${component.price:,.2f}</td>
            <td class="small-text" data-column="5" data-name="price">${(component.price * component.quantity):,.2f}</td>
            </tr>"""
        ...
        <th class="small-text" data-column="3" data-name="qty"></th>
        <th class="small-text" data-column="4" data-name="unit-price"></th>
        <th class="small-text" data-column="5" data-name="price">Total: ${self.get_total_cost():,.2f}</th>
        </tr></tfoot></table>"""
        return html
...
class AssemblyDiv:
    ...
    def generate(self) -> str:
        ...
        if self.assembly.laser_cut_parts:
            ...
            laser_cut_table = LaserCutPartsTable(self.assembly.laser_cut_parts)
    ...
    def generate(self) -> str:
        ...
        <header>
            ...
            <nav class="tabbed primary-container">
                <a class="active primary" data-target="quote">
                    <i>request_quote</i>
                    <span>Quote</span>
                </a>
                <a data-target="workorder">
                    <i>manufacturing</i>
                    <span>Workorder</span>
                </a>
                <a data-target="packing-slip">
                    <i>receipt_long</i>
        ...
        if self.job.get_all_assemblies():
        ...
        html += '<div id="parts-layout"><div class="page-break"></div>'
        html += '''<div class="tabs">
            <a class="active" data-ui="#assemblies-layout"><i>table_view</i>Assemblies Layout</a>
            <a data-ui="#parts-list"> <i>format_list_bulleted</i>Parts List</a>
        </div>'''
