...
class AssemblyTable:
    ...
    def generate(self) -> str:
        ...
        for assembly, data in self.data.items():
            ...
            html += (
                '<tr>'
                f'<td class="ui-table-cell-visible">{image_html}</td>'
...
class ItemsTable:
    def __init__(self, assembly: Assembly, assembly_quantity: int, show_all_items: bool):
        self.headers = ['Part Name', 'Material', 'Thickness', 'Qty', 'Shelf #', 'Notes', 'Flow Tag']
...
class GeneratePrintout:
    def __init__(self, admin_workspace: Workspace, filename: str, title: str, data: dict[Assembly, dict[dict[str, int], dict[str, bool]]], order_number: int) -> None:
    ...
    def generate(self):
        ...
        def get_items_table(assembly: Assembly) -> str:
            ...
            if assembly in list(self.data.keys()):
                ...
                assembly_image_path = assembly.get_assembly_data("assembly_image")
                items_table = ItemsTable(assembly, self.data[assembly]['quantity'], self.data[assembly]['show_all_items'])
