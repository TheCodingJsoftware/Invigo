...
from PyQt6.QtCore import QRunnable, Qt, QThread, QTimer
...
class InventoryItemLoader(QRunnable):
    ...
    def load_item(self, category_data) -> None:
        ...
        try:
            ...
            self._timer.stop()
            set_status_button_stylesheet(button=self.parent.status_button, color="#3daee9")
            ...
            part_number: str = self.parent.get_value_from_category(item_name=item, key="part_number")
            group = self.parent.get_value_from_category(item_name=item, key="group")
            try:
                current_quantity: int = int(self.parent.get_value_from_category(item_name=item, key="current_quantity"))
            except TypeError:
                self._timer.stop()
                return
            # Checking if the item is in the inventory.
            if self.parent.get_value_from_category(item_name=item, key="current_quantity") is None:
                return

            # self.item_layouts.append(layout)
            unit_quantity: float = float(self.parent.get_value_from_category(item_name=item, key="unit_quantity"))
            priority: int = self.parent.get_value_from_category(item_name=item, key="priority")
            price: float = self.parent.get_value_from_category(item_name=item, key="price")
            notes: str = self.parent.get_value_from_category(item_name=item, key="notes")
            use_exchange_rate: bool = self.parent.get_value_from_category(item_name=item, key="use_exchange_rate")
            exchange_rate: float = self.parent.get_exchange_rate() if use_exchange_rate else 1
            ...
            latest_change_part_number: str = self.parent.get_value_from_category(item_name=item, key="latest_change_part_number")
            latest_change_unit_quantity: str = self.parent.get_value_from_category(item_name=item, key="latest_change_unit_quantity")
            latest_change_current_quantity: str = self.parent.get_value_from_category(item_name=item, key="latest_change_current_quantity")
            latest_change_price: str = self.parent.get_value_from_category(item_name=item, key="latest_change_price")
            latest_change_use_exchange_rate: str = self.parent.get_value_from_category(item_name=item, key="latest_change_use_exchange_rate")
            latest_change_priority: str = self.parent.get_value_from_category(item_name=item, key="latest_change_priority")
            latest_change_notes: str = self.parent.get_value_from_category(item_name=item, key="latest_change_notes")
            latest_change_name: str = self.parent.get_value_from_category(item_name=item, key="latest_change_name")
            ...
            if current_quantity > 20:
                ...
                spin_current_quantity.setStyleSheet(f"color: {quantity_color}; border-color: {quantity_color}; {self.margin_format}")
            ...
            # PRICE
            round_number = lambda x, n: eval('"%.' + str(int(n)) + 'f" % ' + repr(int(x) + round(float("." + str(float(x)).split(".")[1]), n)))
            converted_price: float = price * self.parent.get_exchange_rate() if use_exchange_rate else price / self.parent.get_exchange_rate()
            spin_price = HumbleDoubleSpinBox(self.parent)
            spin_price.setToolTip(
                "$" + str(round_number(converted_price, 2)) + " CAD" + "\n" + latest_change_price
                if use_exchange_rate
                else "$" + str(round_number(converted_price, 2)) + " USD" + "\n" + latest_change_price
            )
            ...
            # NOTES
            text_notes = NotesPlainTextEdit(parent=self.parent, text=notes, tool_tip=latest_change_notes)
            ...
            btn_delete = DeletePushButton(
                ...
                icon=QIcon(f"ui/BreezeStyleSheets/dist/pyqt6/{self.parent.theme}/trash.png"),
            )
            btn_delete.clicked.connect(partial(self.parent.delete_item, self.category, item_name))
        ...
        for item in list(self.QObjects.keys()):
            ...
            if item.currentText() == self.selected_item:
                if self.highlight_color == "#3daee9":
                    item.setStyleSheet(f"background-color: {self.highlight_color}; border: 1px solid {self.highlight_color};")
                    if spin_current_quantity.value() <= 0:
                        spin_current_quantity.setStyleSheet("background-color: #1d2023; color: red")
                    else:
                        spin_current_quantity.setStyleSheet("background-color: #1d2023; color: white")
                else:
                    if spin_current_quantity.value() <= 0:
                ...
                if spin_current_quantity.value() <= 0:
                    spin_current_quantity.setStyleSheet("background-color: #eff0f1; color: red")
                else:
                    spin_current_quantity.setStyleSheet("background-color: #eff0f1; color: white")
